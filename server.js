...
app.post('/api/editProfile', (req,res) => {
    ...
    if(rank === 'Manager'){
        conn.query(`UPDATE adminUser SET name = '${name}', last_name = '${lastName}', phone = '${phone}', mail = '${mail}', password = '${password}', is_admin = '1' WHERE id = '${id}';`, (err, result) => {
            if (err) {
                console.error('Error: ' + err);
                return;
            }
        ...
        conn.query(`UPDATE employees SET name = '${name}', last_name = '${lastName}', phone = '${phone}', mail = '${mail}', password = '${password}', is_admin = '0' WHERE id = '${id}';`, (err, result) => {
            if (err) {
                console.error('Error: ' + err);
                return;
            }
